...
import { CaslAbilityService } from '../auth/casls/casl-ability.service';
import {
  CHECK_POLICIES_KEY,
  CaslHandlerType,
...
export class CaslGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    if (req.user) {
      ...
      if (handlers) {
        flag = flag && handlers.every((handlers) => handlers(ability));
      }

      if (flag && canHandlers) {
        if (canHandlers instanceof Array) {
          flag = flag && canHandlers.every((handlers) => handlers(ability));
      ...
      if (flag && cannotHandlers) {
        if (cannotHandlers instanceof Array) {
          flag = flag && cannotHandlers.every((handlers) => handlers(ability));
